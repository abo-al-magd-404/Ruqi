export default () => ({
  ...
  jwt: {
    access: {
      ...
      expiresIn: parseInt(process.env.JWT_ACCESS_EXPIRES_IN ?? "10", 10) * 60, // -> 10 minuets
    },

    refresh: {
      secret: process.env.JWT_REFRESH_SECRET,
      expiresIn:
        parseInt(process.env.JWT_REFRESH_EXPIRES_IN ?? "30", 10) * 24 * 60 * 60, // -> 30 days
    },
  },
  ...
  emailVerification: {
    otpExpiresIn:
      parseInt(process.env.EMAIL_VERIFICATION_OTP_EXPIRES_IN ?? "10", 10) * 60, // 10 minutes

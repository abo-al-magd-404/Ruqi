...
export const envValidationSchema = Joi.object({
  ...
  JWT_ACCESS_EXPIRES_IN: Joi.number().required(), // 10 minutes

  JWT_REFRESH_SECRET: Joi.string().min(32).required(),
  JWT_REFRESH_EXPIRES_IN: Joi.number().required(), // 30 days
  ...
  EMAIL_VERIFICATION_OTP_EXPIRES_IN: Joi.number().required(), // 10 minutes
  EMAIL_VERIFICATION_OTP_RESEND_COOLDOWN: Joi.number().required(), // 60 seconds
});

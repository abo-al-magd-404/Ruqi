...
export async function getEducationalMonthById(
  id: string,
): Promise<EducationalMonth> {
...
export async function reorderMonths(items: ReorderItem[]): Promise<void> {
  try {
    await authedWrite(
      `${API_BASE_URL}/educational-content/months/reorder`,
      "PATCH",
      { items },
    );
  } catch (err) {
    ...
    for (const item of items) {
      await authedJson(
        `${API_BASE_URL}/educational-content/months/${item.id}`,
        "PATCH",
        {
          order: item.order,
        },

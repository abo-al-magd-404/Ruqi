...
export function studentStageId(student: AdminStudent | null): string | null {
  ...
  if (student.stage && typeof student.stage === "object")
    return String(student.stage._id);
...
export function studentStageTitle(student: AdminStudent | null): string {
  if (!student) return "—";
  if (student.stage && typeof student.stage === "object")
    return String(student.stage.title);
  return "—";
}
  ...
  const loadStudents = useCallback(async () => {
    try {
      ...
      setSelected((current) => {
        ...
        return (
          data.find((s) => s.studentId === current.studentId) ?? data[0] ?? null
        );
      });
    } catch (e) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (active)
          setStageMonths([...months].sort((a, b) => a.order - b.order));
      } catch {
        if (active) setStageMonths([]);
      } finally {
      ...
      try {
        ...
        const patch = (s: AdminStudent): AdminStudent =>
          s.studentId === studentId
            ? { ...s, subscribedMonths: updatedMonths }
            : s;

...
export default function AdminDashboard({
  ...
  const handleEditSave = async (
    studentId: string,
    payload: Parameters<typeof dash.saveStudent>[1],
  ) => {
  ...
  return (
    <main className="w-full bg-background flex flex-col items-center px-4 py-8 md:px-20 md:py-12 font-cairo gap-10">
      <section className="w-full max-w-7xl bg-surface border border-border rounded-[16px] p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm">
        ...
        <div className="text-[14px] text-text-muted">
          لوحة إدارة الطلاب والاشتراكات
        </div>
      </section>

      <section className="w-full max-w-7xl grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      {dash.error && (
        <div className="w-full max-w-7xl bg-danger-bg border border-danger text-danger font-bold text-[14px] rounded-xl p-4 text-center">
          {dash.error}
        </div>
      )}

      <section className="w-full max-w-7xl flex flex-col lg:flex-row items-start gap-6">
        <aside className="w-full lg:w-90 bg-surface border-[1.5px] border-primary shadow-[0px_8px_24px_-2px_rgba(84,70,58,0.05)] rounded-[20px] p-6 flex flex-col gap-6 shrink-0">
          <div className="flex justify-between items-center border-b border-border pb-4">
            ...
            {selected && (
              <span
                className={`px-3 py-1 rounded-md text-[12px] font-bold ${STATUS_CLASS[selected.status]}`}
              >
          ...
          {selected ? (
            <>
              <div className="flex flex-col items-center gap-4">
                <div className="w-18 h-18 rounded-full bg-background border border-border overflow-hidden flex items-center justify-center text-primary font-black text-[22px]">
                  {selected.avatar ? (
                    <Image
                      src={selected.avatar}
                      alt={selected.name}
              ...
              <div className="flex flex-col gap-2.5">
                <h4 className="text-[14px] font-bold text-text-main">
                  نسبة تفعيل اشتراكات المرحلة
                </h4>
                <div className="flex items-center gap-3">
                  <span className="text-[12px] text-text-muted w-10">
                    {subscriptionRatio}٪
                  </span>
                  <div
                    className="flex-1 bg-primary-light rounded-full h-2 overflow-hidden flex"
                    dir="ltr"
                  >
                    <div
                  ...
                  {(["ACTIVE", "PENDING", "SUSPENDED"] as UserStatus[]).map(
                    (status) => (
                      <button
                        ...
                        onClick={() =>
                          selected.studentId &&
                          dash.changeStatus(selected.studentId, status)
                        }
            ...
            <p className="text-[14px] text-text-muted text-center py-10">
              اختر طالباً من القائمة لعرض تفاصيله
            </p>
          )}
        </aside>
        ...
        <div className="w-full flex flex-col gap-8">
                ...
                <tbody className="text-[14px]">
                  {dash.loading ? (
                    <tr>
                      <td
                        colSpan={5}
                        className="py-10 text-center text-text-muted font-bold"
                      >
                        جاري تحميل الطلاب...
                      </td>
                    </tr>
                    ...
                    dash.filteredStudents.map((student) => {
                      ...
                      return (
                        ...
                        >
                          <td
                            className="py-4 px-6 text-primary font-bold text-[12px]"
                            dir="ltr"
                          >
                            {student.studentId || "—"}
                          </td>
                          ...
                          <td className="py-4 px-6">
                            <div className="flex items-center justify-center gap-2">
                              <ActionButton
                                type="view"
                                label="عرض"
                                onClick={() => dash.setSelected(student)}
                              />
                              <ActionButton
                                type="edit"
                                label="تعديل"
          ...
          <div className="flex flex-col gap-4">
            <h2 className="text-[20px] font-extrabold text-text-main">
              التحكم في وصول الاشتراكات
            </h2>
            ...
            <div className="bg-surface border border-border rounded-[16px] p-6 flex flex-col gap-5 shadow-sm">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-border pb-4 gap-2">
                ...
                <span className="text-[14px] text-text-muted">
                  تحديد الأشهر النشطة لملف الطالب الحالي
                </span>
              </div>
              ...
              {!selected ? (
                ...
                <p className="text-[14px] text-text-muted text-center py-8">
                  جاري تحميل أشهر المرحلة...
                </p>
              ) : dash.stageMonths.length === 0 ? (
                <p className="text-[14px] text-text-muted text-center py-8">
                  لا توجد أشهر متاحة لهذه المرحلة. تأكد من ربط الطالب بمرحلة
                  دراسية.
                </p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {dash.stageMonths.map((month) => {
                    const isSubscribed = subscribedIds.includes(
                      String(month._id),
                    );
                    return (
                      <div
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          <span
                            className={`text-[12px] font-bold ${isSubscribed ? "text-success" : "text-text-muted"}`}
                          >
                          ...
                          <button
                            ...
                            aria-label={
                              isSubscribed ? "إلغاء الاشتراك" : "تفعيل الاشتراك"
                            }
                          >
      ...
      {confirmDelete && (
        <div
          ...
          dir="rtl"
        >
          <div className="w-full max-w-105 bg-surface rounded-[20px] border border-border p-6 shadow-2xl flex flex-col gap-5">
            <h3 className="text-[18px] font-extrabold text-text-main text-center">
              تأكيد حذف الحساب
            </h3>
            <p className="text-[14px] text-text-muted text-center">
              سيتم حذف حساب «{confirmDelete.name}» نهائياً. لا يمكن التراجع عن
              هذا الإجراء.
            </p>
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={dash.busy}
                className="flex-1 h-12 bg-danger text-white font-bold text-[14px] rounded-xl hover:opacity-90 transition-opacity disabled:opacity-60 flex items-center justify-center gap-2"
              >
                <X size={16} />

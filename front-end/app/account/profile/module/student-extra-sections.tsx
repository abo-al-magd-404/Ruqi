...
export default function StudentExtraSections({ profile }: { profile: UserProfile }) {
  ...
  useEffect(() => {
    ...
    async function loadProgress() {
      try {
        ...
        const valid = months.filter((m): m is StudentMonthProgress => m !== null);
        ...
        for (const m of valid) {
          ...
          for (const l of m.lessons) {
            if (!l.completed && (l.videoCompleted || l.explanationCompleted || l.homeworkCompleted || l.bookCompleted)) {
              totals.lessonsStarted++;
            }
            if (l.homeworkCompleted) totals.homeworksDone++;
            else if (l.videoCompleted || l.explanationCompleted || l.bookCompleted) totals.homeworksStarted++;
          }
          ...
          const allExamsDone =
            m.summary.totalExams === 0 || m.summary.completedExams === m.summary.totalExams;
    ...
    getOverallTopStudents()
      .then((list) => {
        ...
        const mine = list.find(
          (s) => String(s.id) === String(profile.studentId) || s.name === profile.name,
        );
        if (mine) {
          const top = list[0];
          setRank({
            rank: mine.rank,
            enoughPoints: top ? Math.max(0, top.points - mine.points + 1) : null,
          });
        } else {
          setRank(null);
        }
  ...
  const homeworksNotStarted = t.totalLessons - t.homeworksDone - t.homeworksStarted;
  const examsNotStarted = t.totalExams - t.completedExams - t.examsStarted;
  const monthsPct = t.monthsTotal > 0 ? Math.round((100 * t.monthsDone) / t.monthsTotal) : 0;
  const radius = 56;
  const circumference = 2 * Math.PI * radius;
  const dashOffset = circumference * (1 - overallPct / 100);

  return (
    <div className="w-full max-w-[1200px] flex flex-col gap-12" dir="rtl">
      {/* ============ ACADEMIC PROGRESS ============ */}
      {progressLoading ? (
        <div className="bg-white border border-[#E9E3D8] rounded-[24px] p-10 h-64 animate-pulse" />
      ) : (
      <section className="bg-[#FFFFFF] border border-[#E9E3D8] rounded-[24px] p-6 md:p-10 shadow-[0px_8px_24px_-2px_rgba(84,70,58,0.06)] flex flex-col gap-10">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="w-full lg:w-[340px] bg-[#FAF8F5] rounded-[16px] p-6 flex flex-col items-center justify-center gap-6 shrink-0">
            <div className="relative w-[100px] h-[100px] sm:w-[110px] sm:h-[110px] lg:w-[130px] lg:h-[130px] flex items-center justify-center">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 130 130">
                <circle cx="65" cy="65" r="56" fill="none" stroke="#FAF3E6" strokeWidth="12" />
                <circle
                  cx="65"
                  cy="65"
                  r="56"
            ...
            <div className="flex flex-col items-center text-center gap-2">
              <h3 className="font-bold text-[14px] text-[#2C2621]">إنجاز المناهج العام</h3>
          ...
          <div className="flex-1 flex flex-col gap-8">
            <div className="bg-[#FAF8F5] rounded-[16px] p-6 flex flex-col gap-4 justify-center h-[92px]">
              <div className="flex flex-row justify-between items-center w-full">
                <span className="font-bold text-[14px] text-[#2C2621]">الشهور المنجزة</span>
                <span className="text-[#6E655F] text-[13px]">
                  تم إنجاز {fmt(t.monthsDone)} من أصل {fmt(t.monthsTotal)} شهر
                </span>
              </div>
              <div className="w-full bg-[#FAF3E6] h-[10px] rounded-full overflow-hidden" dir="ltr">
                <div
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="bg-white border border-[#E9E3D8] rounded-[16px] p-5 shadow-sm flex flex-col gap-4">
                <div className="flex justify-between items-center">
                  <h4 className="font-extrabold text-[16px] text-[#2C2621]">الاختبارات الدورية</h4>
                  <span className="text-[18px] text-[#997D21] font-extrabold">{fmt(t.totalExams)}</span>
                </div>
                <hr className="border-[#E9E3D8]" />
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center">
                    <span className="text-[12px] text-[#6E655F]">منجزة:</span>
                    <span className="text-[12px] font-bold text-[#22C55E]">{fmt(t.completedExams)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[12px] text-[#6E655F]">مستمرة:</span>
                    <span className="text-[12px] font-bold text-[#997D21]">{fmt(t.examsStarted)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[12px] text-[#6E655F]">غير مبدؤة:</span>
                    <span className="text-[12px] font-bold text-[#6E655F]">{fmt(examsNotStarted)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-[#E9E3D8] rounded-[16px] p-5 shadow-sm flex flex-col gap-4">
                <div className="flex justify-between items-center">
                  <h4 className="font-extrabold text-[16px] text-[#2C2621]">الواجبات والتطبيقات</h4>
                  <span className="text-[18px] text-[#997D21] font-extrabold">{fmt(t.totalLessons)}</span>
                </div>
                <hr className="border-[#E9E3D8]" />
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center">
                    <span className="text-[12px] text-[#6E655F]">منجزة:</span>
                    <span className="text-[12px] font-bold text-[#22C55E]">{fmt(t.homeworksDone)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[12px] text-[#6E655F]">مستمرة:</span>
                    <span className="text-[12px] font-bold text-[#997D21]">{fmt(t.homeworksStarted)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[12px] text-[#6E655F]">غير مبدؤة:</span>
                    <span className="text-[12px] font-bold text-[#6E655F]">{fmt(homeworksNotStarted)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-[#E9E3D8] rounded-[16px] p-5 shadow-sm flex flex-col gap-4">
                <div className="flex justify-between items-center">
                  <h4 className="font-extrabold text-[16px] text-[#2C2621]">الدروس والفيديوهات</h4>
                  <span className="text-[18px] text-[#997D21] font-extrabold">{fmt(t.totalLessons)}</span>
                </div>
                <hr className="border-[#E9E3D8]" />
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center">
                    <span className="text-[12px] text-[#6E655F]">منجزة:</span>
                    <span className="text-[12px] font-bold text-[#22C55E]">{fmt(t.completedLessons)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[12px] text-[#6E655F]">مستمرة:</span>
                    <span className="text-[12px] font-bold text-[#997D21]">{fmt(t.lessonsStarted)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[12px] text-[#6E655F]">غير مبدؤة:</span>
                    <span className="text-[12px] font-bold text-[#6E655F]">{fmt(lessonsNotStarted)}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="bg-white border border-[#E9E3D8] rounded-[24px] p-6 md:p-10 shadow-[0px_8px_24px_-2px_rgba(84,70,58,0.06)] flex flex-col gap-8 mb-8">
        <div className="flex flex-col items-center text-center gap-3">
          <div className="flex flex-row items-center gap-3">
            <div className="w-[40px] h-[1.5px] bg-[#D4AF37]" />
          ...
          <p className="text-[14px] text-[#6E655F]">موقعك الحالي بين فرسان الضاد وأبطال الأسبوع</p>
        </div>

        <div className="flex flex-col sm:flex-row gap-4">
          <div className="bg-[#FAF8F5] rounded-[16px] p-6 flex flex-col items-center justify-center flex-1 gap-3">
            <div className="w-[48px] h-[48px] rounded-full bg-[#FAF3E6] border-2 border-[#997D21] flex items-center justify-center">
              <Sparkles size={22} className="text-[#997D21]" />
            </div>
            <span className="text-[#6E655F] text-[14px]">مجموع نقاطك</span>
            <span className="font-black text-[24px] text-[#2C2621]">{fmt(t.totalPoints)} نقطة</span>
          </div>
      ...
      <section className="bg-white border border-[#E9E3D8] rounded-[24px] p-6 md:p-10 shadow-[0px_8px_24px_-2px_rgba(84,70,58,0.06)] flex flex-col gap-4 mb-8">
        ...
        <details className="group bg-[#FAF8F5] border border-[#E9E3D8] rounded-[16px] px-5 py-4">
          ...
          <ol className="mt-4 flex flex-col gap-2 text-[14px] leading-relaxed text-[#6E655F] list-decimal list-inside pr-2">
            ...
            <li>ابعت بياناتك (اسمك، الـ ID بتاعك، الشهر اللي انت عاوز تفعله)</li>
            <li>ابعت اسكرين لريسيت الدفع</li>
            <li>الأدمن هيتواصل معاك وهيتأكد من سلامة البيانات ويفتح ليك المحتوى التعليمي الخاص بالشهر المطلوب ويبلغك</li>
            <li>بعد كدا تذاكر وتجتهد يا بطل وتبقى من أبطالنا وطلابنا المميزين</li>
          </ol>
        </details>

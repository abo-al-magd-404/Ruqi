...
function ChooseAvatar() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background px-4 py-6 sm:py-10 relative overflow-hidden font-cairo">
      ...
      <div
        className="relative z-10 w-full max-w-130 bg-surface rounded-3xl border border-border p-6 sm:p-8 md:p-10 shadow-[0_16px_48px_-4px_rgba(84,70,58,0.0588)] flex flex-col items-center"
        dir="rtl"
      >
        <div className="flex flex-col items-center justify-center mb-4 text-center">
          <svg
            className="w-full max-w-105 h-3 md:h-3.5 mb-4"
            viewBox="0 0 504 18"
            fill="none"
          ...
          <p className="text-center text-[13px] md:text-[14px] font-medium text-text-muted leading-relaxed max-w-95">
            اختر اللون والشكل الذي يُعبّر عنك{" "}
            {name ? `، أهلاً بك ${name}!` : ""}
          </p>
        </div>
        ...
        <button
          ...
          disabled={saving}
          className="mt-5 w-full h-12.5 bg-primary rounded-xl text-text-main font-bold text-[15px] md:text-[16px] shadow-[0_12px_32px_-4px_rgba(196,154,69,0.1)] hover:bg-primary-hover disabled:opacity-60 transition-colors flex items-center justify-center"
        >

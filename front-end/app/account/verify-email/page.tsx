...
export default function VerifyOTP() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background px-4 py-12 relative overflow-hidden font-cairo">
      ...
      <div
        className="relative z-10 w-full max-w-130 bg-surface rounded-3xl border border-border p-8 sm:p-10 md:p-12 shadow-[0_16px_48px_-4px_rgba(84,70,58,0.0588)]"
        dir="rtl"
      >
        <div className="flex flex-col items-center justify-center mb-8">
          <svg
            className="w-full max-w-105 h-3.5 md:h-4.5 mb-6"
            viewBox="0 0 504 18"
            fill="none"
          ...
          <p className="text-center text-[14px] md:text-[15px] font-medium text-text-muted leading-relaxed max-w-90">
            قمنا بإرسال رمز تحقق مكون من 6 أرقام إلى بريدك الإلكتروني، الرجاء
            إدخاله أدناه.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col items-center">
          <div
            className="flex items-center justify-center gap-2 sm:gap-3 w-full mb-8"
            dir="ltr"
          >
            {otp.map((digit, index) => (
              <input
                ...
                onPaste={handlePaste}
                className={`w-11.25 h-13.75 sm:w-13.75 sm:h-15.75 text-center text-[22px] font-bold rounded-[11.5px] border-[1.5px] bg-surface text-text-main outline-none transition-all
                  ${
                    digit !== ""
          ...
          <div className="flex items-center justify-center w-full mb-6">
            {secondsLeft > 0 ? (
              ...
              <span className="text-danger font-medium text-[13px]">
                انتهت صلاحية الرمز
              </span>
            )}
          </div>
          ...
          {!email && (
            <p className="w-full text-[14px] text-warning bg-warning-bg border border-warning/30 rounded-lg p-3 mb-6 text-center font-medium">
              ...
              <Link
                ...
                className="text-warning font-bold underline ms-1"
              >
                أعد التسجيل
              </Link>
            </p>
          ...
          <button
            ...
            className="w-full h-14.5 bg-primary rounded-xl text-text-main font-bold text-[16px] shadow-[0_12px_32px_-4px_rgba(196,154,69,0.1)] hover:bg-primary-hover disabled:opacity-60 disabled:cursor-not-allowed transition-colors flex items-center justify-center mb-6"
          >
          ...
          <div className="flex items-center gap-1 mt-2">
            ...
            >
              {cooldownLeft > 0
                ? `إعادة الإرسال بعد (${formatTime(cooldownLeft)})`
                : "إعادة الإرسال"}
            </button>
          </div>
        </form>

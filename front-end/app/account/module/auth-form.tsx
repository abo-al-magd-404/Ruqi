...
function FormField({
  ...
  return (
    <div className="block mb-5">
      ...
      <input className={`${INPUT_CLASS} ${borderClass}`} dir="rtl" {...props} />
    </div>
  );
}
...
function isValidArabicText(value: string, allowDigits = false): boolean {
  ...
  return allowDigits
    ? ARABIC_ADDRESS_RE.test(value)
    : ARABIC_NAME_RE.test(value);
}
...
export default function AuthForm({ mode: initialMode }: { mode: Mode }) {
  ...
  useEffect(() => {
    if (!isLogin) {
      getEducationalStages()
        ...
        .catch(() =>
          setError("تعذر تحميل المراحل الدراسية، تأكد من اتصال الخادم"),
        )
  ...
  const showNameMsg =
    fieldValidity !== null && isFilled(form.name) && !fieldValidity.name;
  const showPhoneMsg =
    fieldValidity !== null &&
    isFilled(form.phoneNumber) &&
    !fieldValidity.phone;
  const showAddressMsg =
    fieldValidity !== null && isFilled(form.address) && !fieldValidity.address;
  ...
  const passwordsMatch =
    passwordFilled && confirmFilled && form.password === form.confirmPassword;
  ...
  return (
    ...
    >
      ...
      <form
        ...
        className={`relative z-10 w-full bg-surface rounded-[20px] md:rounded-3xl border border-border shadow-[0_16px_48px_-4px_rgba(84,70,58,0.0588)] p-6 sm:p-8 md:p-12 transition-all ${
          isLogin ? "max-w-130" : "max-w-150"
        }`}
        dir="rtl"
      >
        <div className="flex flex-col items-center justify-center mb-6 md:mb-8">
          <svg
            className="w-full max-w-126 h-3.5 md:h-4.5 mb-3"
            viewBox="0 0 504 18"
            fill="none"
        ...
        {!isLogin && (
          <>
            <FormField
              ...
              tone={
                fieldValidity
                  ? toneOf(form.name, fieldValidity.name)
                  : "default"
              }
            />
            {showNameMsg && (
        ...
        <PasswordField
          ...
          tone={
            fieldValidity
              ? toneOf(form.password, fieldValidity.password)
              : "default"
          }
        />
        ...
        {showPasswordMsg && (
          <p className="text-danger text-[12px] font-semibold -mt-4 mb-5 text-right">
            كلمة المرور يجب أن تحتوي على 8 أحرف على الأقل وتتضمن حرفاً كبيراً
            وصغيراً ورقمية ورمزاً خاصاً
          </p>
        )}
        ...
        {!isLogin && (
          <>
            <FormField
              ...
              tone={
                fieldValidity
                  ? toneOf(form.phoneNumber, fieldValidity.phone)
                  : "default"
              }
            />
            {showPhoneMsg && (
            ...
            <FormField
              ...
              onChange={handleChange}
              required
              placeholder="مثال: الجيزة , حي الاهرامات"
              tone={
                fieldValidity
                  ? toneOf(form.address, fieldValidity.address)
                  : "default"
              }
            />
            {showAddressMsg && (
            ...
            <Link
              href={
                activationPendingEmail
                  ? `/account/verify-email?email=${activationPendingEmail}`
                  : "/account/register"
              }
        ...
        <div className="flex flex-col items-center gap-5 mt-2">
          <button
            ...
            className="w-full h-13 md:h-14.5 bg-primary rounded-xl text-text-main font-bold text-[15px] md:text-[16px] shadow-[0_12px_32px_-4px_rgba(196,154,69,0.1)] hover:bg-primary-hover disabled:opacity-60 transition-colors flex items-center justify-center"
          >
            {loading
              ? isLogin
                ? "جاري الدخول..."

...
export default function PasswordField({
  ...
  return (
    <div className="block mb-6">
      <label>
        <span className="block text-[13px] md:text-[14px] font-semibold text-text-main mb-2 text-right">
          {label}
        </span>
        <div className="relative">
          <input
            type={showPassword ? "text" : "password"}
            className={`${INPUT_CLASS} pl-14 ${borderClass}`}
            dir="rtl"
            {...props}
          />
          <button
      ...
      {showStrength && (
        <div className="flex flex-col gap-1.5 mt-3">
          <div className="flex justify-between items-center w-full" dir="rtl">
            <span className="text-text-muted font-medium text-[11px] md:text-[12px]">
              مستوى أمان كلمة المرور
            </span>
            <span
              className="font-bold text-[11px] md:text-[12px] transition-colors"
          ...
          <div className="flex flex-row-reverse gap-1 w-full h-1">
            {[1, 2, 3, 4].map((i) => (
              <div
                ...
                style={{
                  backgroundColor: i <= level ? levelColor : trackColor,
                }}
              />
            ))}
          </div>
          <div className="flex flex-col gap-1 mt-1 text-right" dir="rtl">
            ...
            >
              • يجب أن تتضمن حرفاً كبيراً وصغيراً ورقم ورمزاً خاصاً مثل (#, @,
              $)
            </span>
          </div>
        </div>

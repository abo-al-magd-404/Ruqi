...
function optionsOf(value: string, fallbackName: string): AvatarOptions {
  return (
    parseAvatarValue(value) ?? { seed: fallbackName || AVATAR_FALLBACK_NAME }
  );
}
...
export default function AvatarPicker({
  ...
  const update = (next: AvatarOptions) =>
    onChange(avatarStringFromOptions(next));

  const toggleColor = (color: string) => {
    update({
      ...opts,
      seed,
      glyphColor: opts.glyphColor === color ? undefined : color,
    });
  };
  ...
  const toggleShape = (variant: GlyphsShapeVariant) => {
    update({
      ...opts,
      seed,
      shapeVariant: opts.shapeVariant === variant ? undefined : variant,
    });
  };
  ...
  return (
    <div className="w-full flex flex-col items-center gap-4 py-0 max-w-full">
      ...
      >
        <Image
          src={avatarDataUri(opts, previewSize)}
          alt="معاينة الصورة الرمزية"
          width={previewSize}
          height={previewSize}
          unoptimized
        ...
        >
          {GLYPHS_SHAPE_VARIANTS.map((variant) => (
            <button
              ...
              title={variant}
              className={`shrink-0 w-12 h-12 sm:w-13 sm:h-13 rounded-xl overflow-hidden border-2 transition-all bg-surface ${
      ...
      <button
        type="button"
        onClick={randomize}
        className="flex items-center justify-center gap-2 h-11 w-full rounded-xl bg-primary text-text-main font-bold text-[14px] hover:bg-primary-hover transition-colors"
      >
        <Shuffle size={18} />
        توليد عشوائي
      </button>

...
export default function AssignmentPage({
  ...
  if (isLocked) {
    return (
      <div
        ...
        dir="rtl"
      >
        <h3 className="font-extrabold text-[24px] text-text-main mb-3">
          هذا الواجب غير متاح حالياً
        </h3>
        <p className="font-medium text-sm text-text-muted mb-6 text-center max-w-md">
          أكمل المحتوى السابق أولاً، ثم سيفتح لك هذا الواجب التطبيقي إتماماً
          لخطتك الدراسية.
        </p>
        <Link
          href={
        ...
        <Link
          ...
          className="h-12 px-8 mt-3 bg-transparent border border-border rounded-control text-text-muted font-bold text-[15px] hover:bg-surface-secondary transition-colors flex items-center justify-center"
        >
    ...
    if (total > 0) {
      questions.forEach((q, idx) => {
        if (
          ...
          isExactSet(answers[idx], q.correctAnswers)
        ) {
          correct += 1;
        }
      });
  ...
  return (
    <div
      ...
      dir="rtl"
    >
      <main className="flex flex-col items-start gap-6 md:gap-8 w-full max-w-7xl flex-1">
        <ContentBreadcrumb />
          ...
          <div className="flex flex-col items-start gap-6 md:gap-8 flex-1 w-full min-w-0">
            ...
            <div className="w-full bg-surface border border-border shadow-sm rounded-card p-5 sm:p-8 md:p-10 flex flex-col gap-6 md:gap-8">
              <div className="flex flex-col gap-4 w-full">
                <div className="flex justify-between items-center w-full flex-wrap gap-2">
                  <span className="font-bold text-[13px] sm:text-[14px] text-primary">
                    التقدم: {progressPercentage}٪ ({currentQuestionIndex + 1} من{" "}
                    {totalQuestions})
                  </span>
              ...
              {currentQuestion ? (
                <div className="flex flex-col gap-6 md:gap-8 w-full">
                  <h2 className="font-bold text-[16px] sm:text-[18px] md:text-[20px] text-text-main leading-relaxed w-full wrap-break-word">
                    ...
                    {currentQuestion.options.map((option, optIdx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-start sm:items-center justify-between w-full min-h-14 sm:min-h-16 p-4 sm:px-4.5 rounded-xl transition-all duration-200 text-right border-[1.5px] ${
                            isSelected
                        ...
                        >
                          <div className="flex items-start sm:items-center gap-3 flex-1 min-w-0">
                            <div
                              className={`w-5 h-5 sm:w-5.5 sm:h-5.5 rounded-md flex items-center justify-center shrink-0 border-2 transition-colors mt-0.5 sm:mt-0 ${
                                isSelected
                            ...
                            <span
                              className={`font-medium text-[14px] sm:text-[15px] leading-relaxed wrap-break-word flex-1 text-right ${isSelected ? "text-primary-hover" : "text-text-main"}`}
                            >
                              {option}
                            </span>
                          </div>
            ...
            <div className="w-full flex flex-row items-start gap-4 p-4 sm:p-5 bg-warning-bg border border-warning/30 rounded-card">
              ...
              <div className="flex flex-col gap-1 flex-1 min-w-0">
                <h4 className="font-bold text-[14px] sm:text-[15px] text-warning">
                  مطلوب مذاكرته من الكتاب
                </h4>
                <p className="font-medium text-[12px] sm:text-[13px] text-warning leading-relaxed wrap-break-word">
                  يرجى مراجعة المقرر المطبوع أو صفحات الكتاب المدرسي للتمكن من
                  فهم أحكام وقواعد الدرس بشكل صحيح قبل استكمال الواجب.
                </p>
              </div>
            </div>
            ...
            <Link
              ...
              className="w-full h-13 bg-surface hover:bg-primary-light border border-border text-text-main font-bold text-[14px] sm:text-[15px] rounded-xl transition-colors flex items-center justify-center"
            >
              العودة إلى الدرس
            </Link>
          </div>

...
import MonthDrawer, {
  MonthDrawerButton,
} from "@/app/educational-content/module/MonthDrawer";
...
export default function ContentDetailsPage({
      ...
      try {
        ...
        if (contentData && contentData.month) {
          ...
          if (profile?.role === "STUDENT") {
            const monthProgress = await getMonthProgress(
              contentData.month,
            ).catch(() => null);
            if (!active) return;
            if (
  ...
  if (isLocked) {
    return (
      <div
        ...
        dir="rtl"
      >
        <h3 className="font-extrabold text-2xl text-text-main mb-3">
        ...
        <Link
          href={
            content?.month
              ? `/educational-content/month/${content.month}`
              : "/educational-content"
          }
          className="h-12 px-8 bg-primary rounded-control text-surface font-bold text-base hover:bg-primary-hover transition-colors flex items-center justify-center"
        >
          العودة إلى المحتوى
        </Link>
        <Link
          href="/educational-content"
          className="h-12 px-8 mt-3 bg-transparent border border-border rounded-control text-text-muted font-bold text-base hover:bg-surface-secondary transition-colors flex items-center justify-center"
        >
  ...
  if (!content) {
    return (
      <div
        ...
        dir="rtl"
      >
        <h3 className="font-extrabold text-2xl text-danger mb-3">
          عذراً، المحتوى غير متوفر
        </h3>
        <Link
          href="/educational-content"
          className="h-12 px-8 bg-primary rounded-control text-surface font-bold text-base hover:bg-primary-hover transition-colors flex items-center justify-center"
  ...
  const currentIndex = monthContentList.findIndex(
    (item) => item._id === content._id,
  );
  const contentPosition =
    currentIndex === -1 ? content.order : currentIndex + 1;
  const totalItems = monthContentList.length || 1;
  const nextItem =
    currentIndex < monthContentList.length - 1 && currentIndex !== -1
      ? monthContentList[currentIndex + 1]
      : null;

  const articleText = content.writtenExplanation || content.description || "";
  const embedVideoUrl = content.videoUrl
    ? toEmbedVideoUrl(content.videoUrl)
    : null;
  const drawerLockedIds = isStudent
    ? getSequenceLockedIds(monthContentList, monthProgress)
    : new Set<string>();
  const currentLessonStages = isStudent
    ? {
        video: !content.videoUrl || lessonProgress?.videoCompleted === true,
        explanation:
          !content.writtenExplanation ||
  ...
  return (
    <div
      className="w-full min-h-screen bg-background flex flex-col items-center pt-20 pb-20 lg:pt-24 lg:pb-14 px-4 md:px-20 relative font-cairo"
      dir="rtl"
    >
      <main className="flex flex-col items-start gap-8 w-full max-w-7xl flex-1">
        <ContentBreadcrumb />
          ...
          <div className="flex flex-col items-start gap-6 flex-1 w-full min-w-0">
                ...
                {[
                  ...
                  ...(articleText
                    ? [
                        {
                          label: "الشرح",
                          done: lessonProgress.explanationCompleted,
                        },
                      ]
                    : []),
                  ...(content.note
                    ? [{ label: "الكتاب", done: lessonProgress.bookCompleted }]
                    : []),
                  ...(content.homework && content.homework.length > 0
                    ? [
                        {
                          label: "التدريب",
                          done: lessonProgress.homeworkCompleted,
                        },
                      ]
                    : []),
                ].map((s) => (
            ...
            {step === "explanation" || step === "book" ? (
              <button
                type="button"
                onClick={() =>
                  setStep(step === "book" ? "explanation" : "video")
                }
            ...
            {step === "video" && articleText && (
              <button
                ...
                className="w-full h-14.5 bg-surface border border-primary text-primary font-bold text-[16px] rounded-control transition-colors flex items-center justify-center gap-2 hover:bg-primary-light mt-2"
              >
            ...
            {step === "explanation" && articleText && (
              ...
              >
                ...
                <p className="font-medium text-[15px] text-text-muted leading-8 w-full whitespace-pre-wrap wrap-break-word">
                  {articleText}
                </p>
              </div>
            )}
            {step === "explanation" && content.note && (
              <button
                ...
                className="w-full h-14.5 bg-surface border border-warning text-warning font-bold text-[16px] rounded-control transition-colors flex items-center justify-center gap-2 hover:bg-warning-bg mt-2"
              >
                <span>الانتقال إلى الكتاب المطلوب</span>
                <ChevronDown size={20} strokeWidth={2.5} />
              </button>
            ...
            {step === "book" && content.note && (
              ...
              >
                ...
                <p className="font-medium text-[15px] text-warning leading-8 w-full whitespace-pre-wrap wrap-break-word">
                  {content.note}
                </p>
                <button
                  type="button"
                  ...
                  className={`self-start h-11.5 px-6 font-bold text-[14px] rounded-control transition-opacity ${
            ...
            {(step === "book" ||
              ...
              <button
                ...
                className="w-full h-14.5 bg-primary hover:bg-primary-hover text-footer font-bold text-[16px] rounded-control shadow-[0_12px_32px_rgba(196,154,69,0.1)] transition-colors flex items-center justify-center gap-2 mt-4"
              >
                <span>
                  {content.homework && content.homework.length > 0

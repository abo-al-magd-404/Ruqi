...
export default function StageMonthsPage({
  params,
}: {
      ...
      <main className="flex flex-col items-start px-4 sm:px-2 lg:px-0 gap-8 md:gap-12 w-full max-w-300">
        <div className="flex flex-col items-start gap-4 w-full">
          ...
          <div className="flex flex-col items-start gap-3 w-full mt-2 md:mt-4">
            ...
            <p className="font-medium text-[14px] md:text-[16px] text-text-muted leading-relaxed md:leading-7.5 max-w-164.25">
              اختر الشهر الدراسي للبدء في تصفح الدروس والاختبارات المخصصة لهذه
              المرحلة وارتقِ بمستواك الأكاديمي
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 w-full">
            {months.map((month) => (
              <Link
                href={`/educational-content/month/${month._id}`}
                key={month._id}
              ...
              >
                <div className="flex flex-col w-full h-full bg-surface rounded-card shadow-lg border border-border overflow-hidden cursor-pointer group-hover:scale-[1.02] transition-transform duration-300">
                  <div
                    className="w-full h-45 bg-surface-secondary bg-cover bg-center border-b border-border"
                    style={{
                      backgroundImage: `url('${month.image || "/placeholder.svg"}')`,
                    }}
                  ></div>

                  <div className="flex flex-col grow p-5 md:p-6 gap-3">
                    ...
                    <p className="font-medium text-[13px] md:text-[14px] text-text-muted line-clamp-3 leading-relaxed grow">

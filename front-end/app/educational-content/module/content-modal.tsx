...
const EMPTY_QUESTION: ContentQuestion = {
  questionText: "",
  options: ["", ""],
  correctAnswers: [],
};
...
export default function ContentModal({
  ...
  const [passPercentage, setPassPercentage] = useState(
    String(initialPassPercentage),
  );
  const [questions, setQuestions] =
    useState<ContentQuestion[]>(initialQuestions);
  ...
  if (open !== prevOpen) {
    ...
    if (open) {
      ...
      setQuestions(
        initialQuestions.length ? [...initialQuestions] : [EMPTY_QUESTION],
      );
    }
  }
  ...
  const addOption = (qIndex: number) => {
    setQuestions((q) =>
      q.map((item, i) =>
        i === qIndex ? { ...item, options: [...item.options, ""] } : item,
      ),
    );
  };
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-140 max-h-[90vh] overflow-y-auto scrollbar-hide bg-surface rounded-t-[20px] sm:rounded-b-[20px] border border-border p-6 md:p-8 shadow-2xl"
        dir="rtl"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          {mode === "create" && onSwitchType && (
            <div className="flex gap-2 p-1 bg-surface-secondary rounded-xl">
              <button
                ...
                className={`flex-1 h-10.5 rounded-lg font-bold text-[14px] transition-colors ${type === "LESSON" ? "bg-primary text-text-main" : "text-text-muted hover:bg-surface"}`}
              >
                درس مرئي
              </button>
              <button
                ...
                className={`flex-1 h-10.5 rounded-lg font-bold text-[14px] transition-colors ${type === "EXAM" ? "bg-primary text-text-main" : "text-text-muted hover:bg-surface"}`}
              >
                اختبار
              </button>
            </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              placeholder={
                type === "LESSON"
                  ? "مثال: الدرس الأول - التيار الكهربي"
          ...
          <div className="flex flex-col gap-3 mt-1">
            ...
            {questions.map((q, qi) => (
              <div
                key={qi}
                className="border border-border rounded-xl bg-surface-secondary p-4 flex flex-col gap-3"
              >
                <div className="flex items-start gap-2">
                  <span className="font-extrabold text-[14px] text-primary-hover shrink-0 mt-2.5">
                    {qi + 1}
                  </span>
                  <textarea
                      ...
                      return (
                        <div key={oi} className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            className={`flex-1 h-10.5 rounded-lg border-[1.5px] px-3 text-text-main text-[13px] outline-none transition-colors ${
                              isCorrect
                                ? "border-success bg-success-bg focus:border-success"
            ...
            <button
              type="button"
              onClick={addQuestion}
              className="flex items-center justify-center gap-1 w-full border-[1.5px] border-dashed border-primary text-primary-hover hover:bg-primary-light font-bold text-[13px] h-11 rounded-xl transition-colors"
            >
              <Plus size={16} />
              إضافة سؤال
            </button>

...
export default function MonthDrawer({
  ...
  return (
    ...
    >
      ...
      >
        <div className="w-full max-h-[80vh] bg-background border-t border-border rounded-t-3xl shadow-2xl flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-2.5">
            ...
            {contentList.length === 0 ? (
              ...
              contentList.map((item, index) => {
                ...
                const isLocked =
                  lockedIds?.has(String(item._id)) === true && !isCurrent;
                const href = isLessonItem
                  ? `/educational-content/content/${item._id}`
                  : `/educational-content/exam/${item._id}`;
                const showStages =
                  isCurrent && isLessonItem && !!currentLessonStages;

                const stageChips = [
                  {
                    label: "الفيديو",
                    done: currentLessonStages?.video ?? false,
                  },
                  {
                    label: "الشرح",
                    done: currentLessonStages?.explanation ?? false,
                  },
                  {
                    label: "الكتاب المطلوب",
                    done: currentLessonStages?.book ?? false,
                  },
                  {
                    label: "التدريب",
                    done: currentLessonStages?.training ?? false,
                  },
                ];
                ...
                if (isLocked) {
                  return (
                    <div
                      key={item._id}
                      className={`block w-full cursor-not-allowed ${rowClass}`}
                      aria-disabled="true"
                      dir="rtl"
                    >
                      {rowInner}
                ...
                return (
                  <Link
                    ...
                    key={item._id}
                    onClick={onClose}
                    className={`block w-full ${rowClass}`}
                    dir="rtl"
                  >
                    {rowInner}
                  </Link>
...
export function MonthDrawerButton({ onClick }: { onClick: () => void }) {
  return (
    <div
      className="sticky bottom-4 w-full flex justify-center pointer-events-none lg:hidden mt-auto pt-6 z-1050 font-cairo"
      dir="rtl"
    >
      <button
        type="button"
        onClick={onClick}
        className="pointer-events-auto flex flex-row items-center justify-center gap-2.5 px-8 h-14 bg-footer rounded-full shadow-[0_12px_28px_rgba(0,0,0,0.3)] border border-border/20 transition-transform active:scale-95"

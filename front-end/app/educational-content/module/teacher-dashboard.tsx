...
export default function TeacherDashboard() {
  ...
  return (
    <div
      className="w-full min-h-screen bg-background font-cairo py-10 px-4 md:px-8 lg:px-20 overflow-x-hidden"
      dir="rtl"
    >
      ...
      <ContentModal
        ...
        initialWrittenExplanation={
          dash.editingContent?.writtenExplanation ?? ""
        }
        initialPassPercentage={dash.editingContent?.passPercentage ?? 50}
        initialQuestions={
          dash.editingContent
            ? ((dash.editingContent.type === "EXAM"
                ? dash.editingContent.examQuestions
                : dash.editingContent.homework) ?? [])
            : []
        }
        saving={dash.saving}
        error={dash.modalError}

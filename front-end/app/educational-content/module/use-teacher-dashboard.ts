...
function validateQuestion(q: ContentQuestion): string | null {
  ...
  if (q.correctAnswers.length < 1)
    return "يجب تحديد إجابة صحيحة واحدة على الأقل لكل سؤال";
  if (
    q.correctAnswers.some(
      (c) => !Number.isInteger(c) || c < 0 || c >= q.options.length,
    )
  )
...
export function useTeacherDashboard() {
  ...
  const [editingStage, setEditingStage] = useState<EducationalStage | null>(
    null,
  );
  ...
  const handleStageSave = async (data: StageFormValues) => {
    ...
    try {
      if (stageModalMode === "create") {
        ...
        await updateStage(editingStage._id, {
          title: data.title,
          image: data.image || undefined,
        });
      }
      ...
      setModalError(
        e instanceof Error ? e.message : "حدث خطأ أثناء حفظ المرحلة",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleContentSave = async (data: ContentFormValues) => {
    ...
    try {
      ...
      if (contentModalMode === "create") {
        if (contentType === "LESSON") {
          ...
          if (data.note.trim() && created?._id) {
            await updateContent(created._id, "LESSON", {
              note: data.note.trim(),
            });
          }
        } else {
      ...
      setModalError(
        e instanceof Error ? e.message : "حدث خطأ أثناء حفظ المحتوى",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      if (deleteTarget.kind === "stage") {
        ...
        await deleteContent(
          deleteTarget.content._id,
          deleteTarget.content.type,
        );

  ...
  useEffect(() => {
    ...
    const build = async () => {
      ...
      if (route === "stage" && id) {
        ...
        if (month?.stage) {
          const stage = await getEducationalStageById(
            String(month.stage),
          ).catch(() => null);
          if (stage) {
            trail.push({
              label: stage.title,
              href: `/educational-content/stage/${stage._id}`,
            });
          }
        }
        if (month) {
        ...
        if (content?.month) {
          const month = await getEducationalMonthById(
            String(content.month),
          ).catch(() => null);
          if (month) {
            const stage = month.stage
              ? await getEducationalStageById(String(month.stage)).catch(
                  () => null,
                )
              : null;
            if (stage) {
      ...
      {items.map((item, index) => {
        ...
        return (
          <span key={item.label + index} className="flex items-center gap-2">
            ...
            {isLast ? (
              <span
                className="font-bold text-primary text-xs md:text-sm max-w-55 md:max-w-75 truncate"
                aria-current="page"
              >
                {item.label}
              </span>

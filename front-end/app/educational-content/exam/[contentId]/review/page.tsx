...
export default function ExamReviewPage({
  ...
  useEffect(() => {
    ...
    const fetchContent = async () => {
      try {
        ...
        if (!active) return;
        if (rawAnswers) {
          const parsed = JSON.parse(rawAnswers) as Record<
            string,
            number | number[]
          >;
  ...
  return (
    <div
      ...
      dir="rtl"
    >
      <main className="flex flex-col items-start gap-8 w-full max-w-220 flex-1">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full bg-surface border border-border shadow-sm rounded-card p-6">
          ...
          <Link
            ...
            className="flex items-center justify-center gap-2 h-12 px-6 bg-primary-light text-primary-hover hover:bg-primary hover:text-surface font-bold text-[14px] rounded-xl transition-all shrink-0"
          >
            <ArrowRight size={18} />
        ...
        <div className="flex flex-col gap-8 w-full">
          {questions.map((question, qIndex) => {
            ...
            const isCorrectAnswered =
              chosen.length > 0 && isExactSet(chosen, question.correctAnswers);
            ...
            return (
              ...
              >
                ...
                <h2 className="font-bold text-[18px] text-text-main leading-relaxed w-full wrap-break-word">
                ...
                <div className="flex flex-col gap-3 w-full">
                  {question.options.map((option, optIdx) => {
                    ...
                    if (isCorrectChoice) {
                      optionStyle =
                        "bg-success-bg border-success text-success font-bold shadow-sm";
                      Icon = (
                        <Check
                          size={18}
                      ...
                      optionStyle =
                        "bg-danger-bg border-danger text-danger font-bold shadow-sm";
                      Icon = (
                    ...
                    return (
                      <div
                        key={optIdx}
                        className={`flex items-start sm:items-center justify-between w-full min-h-14 p-4 sm:px-4.5 rounded-xl border-[1.5px] transition-all ${optionStyle}`}
                      >
                        <div className="flex items-start sm:items-center gap-3 flex-1 min-w-0">
                          <span className="text-[14px] sm:text-[15px] leading-relaxed wrap-break-word flex-1 text-right">
                            {option}
                          </span>
                        </div>

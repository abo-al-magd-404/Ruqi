...
function saveAnswers(contentId: string, answers: Record<number, number[]>) {
  try {
    localStorage.setItem(
      getStorageKey(contentId, "answers"),
...
export default function ExamTakingPage({
  ...
  const submitExam = useCallback(async () => {
    ...
    const answers = qs.map((_, idx) =>
      [...(selectedAnswers[idx] ?? [])].sort((a, b) => a - b),
    );
    let correct = 0;
    if (total > 0) {
      qs.forEach((q, idx) => {
      ...
      if (apiError.status === 401 || apiError.status === 403) {
        ...
        setSubmitError(
          error instanceof Error
            ? error.message
            : "تعذر تسليم الاختبار، حاول مجدداً",
        );
        return;
      }
    }
    ...
    try {
      localStorage.setItem(
        `exam_${contentId}_answers`,
        JSON.stringify(selectedAnswers),
      );
      localStorage.setItem(
        `exam_${contentId}_result`,
        JSON.stringify({ score: scorePct, correct, total }),
      );
    } catch {}
    clearExamStorage(contentId);
    router.replace(
      `/educational-content/exam/${contentId}/result?score=${scorePct}&correct=${correct}&total=${total}`,
    );
      ...
      if (document.hidden) {
        setViolations((prev) => {
          ...
          if (next >= MAX_VIOLATIONS) {
            setViolationMessage(
              "تم تجاوز الحد المسموح للمخالفات. سيتم تسليم الاختبار تلقائياً.",
            );
            ...
            setViolationMessage(
              `تنبيه: لقد غادرت صفحة الاختبار! مخالفة ${next} من ${MAX_VIOLATIONS}.`,
            );
            setShowViolationWarning(true);
  ...
  const questions: ContentQuestion[] =
    content.examQuestions || content.homework || [];
  ...
  return (
    <div
      className="w-full min-h-screen bg-background flex flex-col font-cairo overflow-x-hidden"
      dir="rtl"
    >
      {showViolationWarning && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div
            className={`flex flex-col items-center gap-4 p-6 sm:p-8 bg-surface border-2 rounded-[20px] max-w-105 w-full text-center shadow-xl ${
          ...
          >
            ...
            {violations < MAX_VIOLATIONS && (
              <p className="font-medium text-[13px] sm:text-[14px] text-text-muted leading-5.5">
                يُرجى البقاء في صفحة الاختبار حتى انتهاء الوقت أو تسليم الإجابات
                لمنع إلغاء الاختبار.
              </p>
            )}
          </div>
      ...
      <header className="w-full bg-footer px-4 sm:px-8 md:px-20 h-18 sm:h-22 flex flex-row justify-between items-center shrink-0 shadow-md">
        <div className="flex items-center gap-3 sm:gap-4 h-full">
          <svg
            ...
            className="shrink-0 w-11.25 h-11.25 sm:w-15 sm:h-15"
          >
          ...
          <h2 className="hidden md:block font-extrabold text-[16px] lg:text-[18px] text-primary truncate max-w-75">
            {content.title}
          </h2>
        </div>

        <div className="flex items-center px-3 sm:px-5 py-1.5 sm:py-2 bg-danger-bg border border-danger rounded-lg shrink-0">
          <span className="font-bold text-[13px] sm:text-[15px] text-danger">
      ...
      <main className="w-full flex-1 flex flex-col items-center py-6 sm:py-10 md:py-16 px-4 sm:px-8 md:px-20">
        <div className="w-full max-w-270 flex flex-col items-start gap-4 sm:gap-6 md:gap-8">
          {submitError && (
          ...
          <div className="w-full flex flex-col gap-6 sm:gap-8 p-5 sm:p-8 md:p-12 bg-surface border border-border shadow-sm rounded-[20px] md:rounded-card">
            ...
            {currentQuestion ? (
              <div className="flex flex-col gap-6 sm:gap-8 w-full">
                <h2 className="font-bold text-[16px] sm:text-[18px] md:text-[20px] text-text-main leading-relaxed text-right w-full wrap-break-word">
                  ...
                  {currentQuestion.options.map((option, optIdx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-start sm:items-center justify-between w-full min-h-14 sm:min-h-16 p-3.5 sm:p-4 md:px-5 rounded-xl transition-all duration-200 text-right border-[1.5px] ${
                          isSelected
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <div
                            className={`w-5 h-5 sm:w-5.5 sm:h-5.5 rounded-md flex items-center justify-center shrink-0 border-2 transition-colors ${
                              isSelected
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-border mt-2 sm:mt-4">
              ...
              <button
                ...
                onClick={handleNext}
                disabled={
                  currentQuestionIndex >= totalQuestions - 1 && !currentQuestion
                }

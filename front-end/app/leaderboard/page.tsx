    ...
    mySummary && !baseRows.some((r) => r.isCurrentUser)
      ? [
          ...baseRows,
          {
            ...
            rank:
              baseRows.filter((r) => r.points >= mySummary.points).length + 1,
            imageUrl: "",
            isCurrentUser: true,
            ...
            {topThree.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6 w-full justify-items-center items-stretch md:items-end">
                {topThree.find((s) => s.rank === 2) && (
                  <div className="flex flex-col items-center justify-center p-6 gap-4 rounded-[20px] col-span-1 order-2 md:order-1 w-full max-w-75 bg-surface border border-border shadow-lg hover:-translate-y-1 transition-transform duration-300">
                    <div className="relative w-18 h-18 rounded-full bg-background">
                      <Image
                        src={
                          resolveAvatarSrc(
                            topThree.find((s) => s.rank === 2)?.imageUrl,
                    ...
                    <div className="flex flex-col items-center gap-1">
                      <span className="text-[13px] md:text-[16px] font-bold text-text-main text-center leading-snug wrap-break-word line-clamp-2 w-full">
                        {topThree.find((s) => s.rank === 2)?.name}
                      </span>
                  ...
                  <div className="flex flex-col items-center p-8 gap-5 col-span-2 order-1 md:col-span-1 md:order-2 w-full max-w-[320px] bg-[#1E1A17] shadow-[0_12px_32px_-4px_rgba(212,175,55,0.1)] rounded-3xl z-10 hover:-translate-y-2 transition-transform duration-300">
                    <div className="relative w-24 h-24 rounded-full bg-background border-2 border-primary">
                      <Image
                        src={
                          resolveAvatarSrc(
                            topThree.find((s) => s.rank === 1)?.imageUrl,
                    ...
                    <div className="flex flex-col items-center gap-1.5">
                      <span className="text-[16px] md:text-[20px] font-extrabold text-white text-center leading-snug wrap-break-word line-clamp-2 w-full">
                        {topThree.find((s) => s.rank === 1)?.name}
                      </span>
                ...
                {topThree.find((s) => s.rank === 3) && (
                  <div className="flex flex-col items-center justify-center p-6 gap-4 rounded-[20px] col-span-1 order-3 w-full max-w-75 bg-surface border border-border shadow-lg hover:-translate-y-1 transition-transform duration-300">
                    <div className="relative w-18 h-18 rounded-full bg-background">
                      <Image
                        src={
                          resolveAvatarSrc(
                            topThree.find((s) => s.rank === 3)?.imageUrl,
                ...
                <div className="hidden md:block w-full rounded-[20px] border border-border shadow-sm bg-surface overflow-hidden">
                  <table className="w-full table-fixed text-center border-collapse">
                    <thead className="bg-primary-light">
                      <tr>
                        <th className="py-5 px-4 font-bold text-[14px] text-text-main w-[12%]">
                          المركز
                        </th>
                        <th className="py-5 px-4 font-bold text-[14px] text-text-main w-[38%]">
                          اسم الطالب
                        </th>
                        <th className="py-5 px-4 font-bold text-[14px] text-text-main w-[25%]">
                          المرحلة الدراسية
                        </th>
                        <th className="py-5 px-4 font-bold text-[14px] text-text-main w-[25%]">
                          مجموع النقاط
                ...
                <div className="md:hidden flex flex-col gap-3 w-full">
                  {otherRanks.map((student) => (
                    ...
                    >
                      ...
                      <Image
                        src={
                          resolveAvatarSrc(student.imageUrl, student.name).src
                        }
                        alt={student.name}
                        width={56}
                        height={56}

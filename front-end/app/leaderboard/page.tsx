...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`https://api.yourdomain.com/leaderboard?tab=${activeTab}`);
  ...
  return (
    <div dir="rtl" className="w-full min-h-svh bg-background flex flex-col items-center overflow-x-hidden">
      <main className="flex flex-col items-center pt-10 md:pt-16 pb-20 md:pb-[120px] px-4 md:px-8 gap-10 md:gap-14 w-full max-w-[1280px]">
        ...
        <div className="flex flex-col items-center gap-4 w-full text-center">
          ...
          <p className="font-medium text-[14px] md:text-[16px] text-text-muted leading-relaxed md:leading-[26px]">
        ...
        {isLoading ? (
            ...
            {topThree.length > 0 && (
              <div className="flex flex-col md:flex-row justify-center items-center md:items-end gap-6 w-full">
                ...
                {topThree.find(s => s.rank === 2) && (
                  <div className="flex flex-col items-center p-6 gap-4 w-full md:w-[302px] bg-surface border border-border shadow-lg rounded-[20px] order-2 md:order-1 hover:-translate-y-1 transition-transform duration-300">
                    <div className="relative w-[72px] h-[72px] rounded-full bg-background">
                      <div 
                        className="w-full h-full rounded-full bg-cover bg-center"
                        style={{ backgroundImage: `url('${topThree.find(s => s.rank === 2)?.imageUrl}')` }}
                      ></div>
                      <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full flex justify-center items-center ${getRankBadgeColor(2)} text-white font-extrabold text-[12px]`}>
                        ٢
                      </div>
                    </div>
                    <div className="flex flex-col items-center gap-1">
                      <span className="font-bold text-[16px] text-text-main truncate max-w-[200px]">
                ...
                {topThree.find(s => s.rank === 1) && (
                  <div className="flex flex-col items-center p-8 gap-5 w-full md:w-[308px] bg-[#1E1A17] shadow-[0_12px_32px_-4px_rgba(212,175,55,0.1)] rounded-[24px] order-1 md:order-2 z-10 hover:-translate-y-2 transition-transform duration-300">
                    <div className="relative w-[96px] h-[96px] rounded-full bg-background border-2 border-primary">
                      <div 
                        className="w-full h-full rounded-full bg-cover bg-center"
                        style={{ backgroundImage: `url('${topThree.find(s => s.rank === 1)?.imageUrl}')` }}
                      ></div>
                      <div className={`absolute -bottom-1 -right-1 w-8 h-8 rounded-full flex justify-center items-center ${getRankBadgeColor(1)}`}>
                    ...
                    <div className="flex flex-col items-center gap-1.5">
                      <span className="font-extrabold text-[20px] text-white truncate max-w-[220px]">
                ...
                {topThree.find(s => s.rank === 3) && (
                  <div className="flex flex-col items-center p-6 gap-4 w-full md:w-[302px] bg-surface border border-border shadow-lg rounded-[20px] order-3 hover:-translate-y-1 transition-transform duration-300">
                    <div className="relative w-[72px] h-[72px] rounded-full bg-background">
                      <div 
                        className="w-full h-full rounded-full bg-cover bg-center"
                        style={{ backgroundImage: `url('${topThree.find(s => s.rank === 3)?.imageUrl}')` }}
                      ></div>
                      <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full flex justify-center items-center ${getRankBadgeColor(3)} text-white font-extrabold text-[12px]`}>
                        ٣
                      </div>
                    </div>
                    <div className="flex flex-col items-center gap-1">
                ...
                <table className="w-full min-w-[700px] text-center border-collapse">
                  ...
                  <tbody>
                    {otherRanks.map((student) => (
                      ...
                      >
                        <td className={`py-5 px-4 font-extrabold text-[15px] ${student.isCurrentUser ? 'text-primary' : 'text-text-main'}`}>
                          {student.points}
                        </td>
                        ...
                        <td className="py-5 px-4 font-bold text-[15px] text-primary">
                          المركز {student.rank === 4 ? 'الرابع' : student.rank === 5 ? 'الخامس' : student.rank === 6 ? 'السادس' : student.rank === 7 ? 'السابع' : student.rank === 8 ? 'الثامن' : student.rank === 9 ? 'التاسع' : student.rank === 10 ? 'العاشر' : student.rank}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <div className="flex flex-col items-center justify-center w-full py-20 md:py-32 px-5 bg-surface border border-border rounded-card text-center">
            ...
            {/* <p className="font-medium text-[14px] md:text-[16px] text-text-muted max-w-[450px]">
              لم يتم تسجيل أي نقاط أو مراكز في لوحة الشرف حتى الآن، بادر بالتفوق وكن أول المنضمين!
            </p> */}
          </div>
        )}
      </main>
